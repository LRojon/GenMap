...
import ReligionsOverlay from './ReligionsOverlay';
import CulturesOverlay from './CulturesOverlay';
...
const MapCanvas = ({ config, generationId, onMapGenerated, isGenerating, activeTab, climateOpacity = 70, onBiomeHover, onCountryHover, onCityHover }) => {
    ...
    const [voronoiRegionMap, setVoronoiRegionMap] = useState(null);
    const [voronoiPoints, setVoronoiPoints] = useState(null);
    const [mapReligions, setMapReligions] = useState(null);
    const [religionMap, setReligionMap] = useState(null);
    const [mapCultures, setMapCultures] = useState(null);
    ...
    useEffect(() => {
        ...
        const generateMap = async () => {
            ...
            try {
                ...
                // Stocker les données Voronoi pour l'overlay
                setVoronoiRegionMap(mapInstance.voronoiRegionMap);
                setVoronoiPoints(mapInstance.voronoiPoints);
                
                // Stocker les religions
                setMapReligions(mapInstance.religions);
                if (mapInstance.religionSystem && mapInstance.religionSystem.religionMap) {
                  setReligionMap(mapInstance.religionSystem.religionMap);
                }

                // Stocker les cultures
                setMapCultures(mapInstance.cultures);
                if (mapInstance.religionSystem && mapInstance.religionSystem.culturePixelMap) {
                  setCultureMap(mapInstance.religionSystem.culturePixelMap);
                }
                ...
                onMapGenerated({
                    ...
                    religions: mapInstance.religions,
                    cultures: mapInstance.cultures,
                    religionSystem: mapInstance.religionSystem,
                });
            } catch (error) {
                if (!abortController.signal.aborted) {
    ...
    return (
        <div className="map-canvas-container">
            ...
            {voronoiRegionMap && voronoiPoints && (
                <VoronoiOverlay 
                    voronoiRegionMap={voronoiRegionMap} 
                    voronoiPoints={voronoiPoints} 
                    config={config} 
                    scale={config.scale}
                    showRegions={activeTab === 'generation' && config.showRegions}
                />
            )}
            {voronoiRegionMap && voronoiPoints && mapReligions && (
                <ReligionsOverlay 
                    religions={mapReligions}
                    voronoiRegionMap={voronoiRegionMap}
                    voronoiPoints={voronoiPoints}
                    config={config}
                    activeTab={activeTab}
            ...
            {voronoiRegionMap && voronoiPoints && mapCultures && (
                <CulturesOverlay 
                    cultures={mapCultures}
                    voronoiRegionMap={voronoiRegionMap}
                    voronoiPoints={voronoiPoints}
                    config={config}
                    activeTab={activeTab}
